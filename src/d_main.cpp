...
// One iteration of the main frame loop. Extracted so the Emscripten build
// can hand it to emscripten_set_main_loop_arg instead of blocking inside
// a for(;;) — browsers don't let us monopolise the event loop.
static void D_DoomLoop_Iteration(int& lasttic)
{
	try
	{
...
// Declared in src/common/platform/emscripten/i_emsc_fs.cpp — kicks IDBFS
// -> IndexedDB via an EM_ASM; async, but scheduled before we unwind so the
// sync completes while the runtime is still alive.
extern "C" void uzdoom_sync_saves();

static int s_emsc_lasttic = 0;

// Signal the JS shell that the engine is shutting down and why. Hides the
// canvas, shows the "Game exited" panel, and kicks a final IDBFS -> IDB
// flush. Safe to call even if the JS side hasn't wired onEngineExit — the
// try/catch swallows ReferenceError.
static void D_NotifyJSExit(int code, const char* reason)
{
	EM_ASM({
		try {
			if (typeof Module !== 'undefined' && typeof Module.onEngineExit === 'function') {
				Module.onEngineExit($0, UTF8ToString($1));
			}
		} catch (e) { console.warn('onEngineExit failed:', e); }
	}, code, reason ? reason : "");
}

static void D_DoomLoop_EmscTick(void*)
{
	if (wantToRestart)
	{
		// Restart is a rare flow (engine 'restart' CCMD). In the browser
		// we don't have a clean way to unwind out of set_main_loop back
		// into the outer GameMain() restart loop — so for v1, just stop
		// the tick and let the user reload the page. The JS shell's
		// onEngineExit handler turns this into a visible "Game exited"
		// panel with a one-click reload button.
		wantToRestart = false;
		emscripten_cancel_main_loop();
		try { M_SaveDefaultsFinal(); } catch (...) {}
		uzdoom_sync_saves();
		D_NotifyJSExit(0, "restart");
		return;
	}

	// CExitEvent is how the engine signals "the user picked Quit" (and a
	// few other bail-out paths). Under a native build this unwinds all the
	// way out of D_DoomLoop into GameMain()'s catch at d_main.cpp:3874,
	// which then runs D_Cleanup/shutdowns/M_SaveDefaultsFinal and returns
	// from main(). In the browser, simulate_infinite_loop=1 already
	// unwound GameMain's frame with a JS throw, so that outer catch is
	// UNREACHABLE from here — we have to do the persistence ourselves.
	//
	// What matters on clean exit, in order of user-visible impact:
	//   1. M_SaveDefaultsFinal writes the config INI to MEMFS.
	//   2. uzdoom_sync_saves flushes MEMFS -> IndexedDB so those writes
	//      survive a tab close / reload.
	//   3. D_NotifyJSExit tells the shell UI to hide the canvas and show
	//      the exit panel. It also does its own second-chance syncfs for
	//      belt-and-braces coverage.
	//   4. exit() unwinds atexit + triggers Module.onExit in JS.
	//
	// GL/audio/GC teardown is skipped on purpose — EXIT_RUNTIME=1 tears
	// the WASM runtime down wholesale a moment later anyway, and running
	// I_ShutdownGraphics/I_ShutdownSound here adds latency without
	// materially improving cleanup (the browser reclaims AL/GL state via
	// context loss the instant the runtime dies).
	try
	{
		D_DoomLoop_Iteration(s_emsc_lasttic);
	}
	catch (const CExitEvent &ev)
	{
		emscripten_cancel_main_loop();
		try { M_SaveDefaultsFinal(); } catch (...) {}
		uzdoom_sync_saves();
		D_NotifyJSExit(ev.Reason(), "quit");
		std::exit(ev.Reason());
	}
	catch (const std::exception &err)
	{
		...
		try { M_SaveDefaultsFinal(); } catch (...) {}
		uzdoom_sync_saves();
		D_NotifyJSExit(-1, err.what());
		// Don't exit() on fatal — leaving the runtime alive lets the shell
		// show the panel with the error message. User can reload to retry.
	}
}
...
#ifdef __EMSCRIPTEN__
	s_emsc_lasttic = 0;
	// simulate_infinite_loop=1 — this throws a JS-level exception that
	// unwinds the C++ stack cleanly while leaving the runtime alive so
	// the tick callback keeps firing from the browser's event loop.
	emscripten_set_main_loop_arg(D_DoomLoop_EmscTick, nullptr, 0, 1);
	// unreachable
#else
	for (;;)
	{
		D_DoomLoop_Iteration(lasttic);
		if (wantToRestart)
		{
			wantToRestart = false;
			return;
